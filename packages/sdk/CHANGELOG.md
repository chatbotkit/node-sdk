...
- 5faf4b2: Refresh the generated platform API contract so SDK request and response types
  match the current GraphQL, task, integration, dataset, and space APIs.
- 5faf4b2: BREAKING: SpaceSite API request and response fields now use `slug` instead of `domain`. Create requests require `slug`; update requests accept an optional `slug`; fetch and list responses expose `slug`.
