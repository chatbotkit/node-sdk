[**@chatbotkit/cli**](../../README.md)

***

[@chatbotkit/cli](../../modules.md) / [input](../README.md) / select

# Function: select()

> **select**(`question`, `options`): [`Promise`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise)\<`string`\>

Defined in: [sdks/node/packages/cli/src/input.js:58](https://github.com/chatbotkit/node-sdk/blob/main/sdks/node/packages/cli/src/input.js#L58)

Ask the user to select from a list of options

## Parameters

### question

`string`

### options

[`SelectOption`](../interfaces/SelectOption.md)[]

## Returns

[`Promise`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise)\<`string`\>
